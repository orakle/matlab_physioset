...
[sens, fids, ext] = read_physioset.sensors.fileName);
...
if numel(hdr.signal) > 1,    
    ...
    sens = read_pns_physioset.sensors.fileName);
    pnsSensors = physioset.sensors.physiology(...
...
try
    ...
    if ~isempty(pnsSensors),
        physioset.sensors.ixed = physioset.sensors.mixed(eegSensors, pnsSensors);
    else
        physioset.sensors.ixed = eegSensors;
    end
