...
% Generate a physioset.sensors.mixed object with sensor information
% =========================================================================
if opt.verbose,
    fprintf('(import) Generating physioset.sensors.descriptions...');
end
eegSensors  = [];
magSensors  = [];
gradSensors = [];
...
if any(isEeg),
    ...
    eegSensors  = physioset.sensors.eeg(...
        'Label',     eegLabels, ...
...
if any(isMag),
    ...
    elseif isfield(hdr.grad, 'pnt'),
        ...
        magCoils = physioset.sensors.coils('Weights', hdr.grad.tra(magIdx,:));
        magSensors  = physioset.sensors.meg(...
            'Coils',        magCoils, ...
...
if any(isGrad),
    ...
    if isfield(hdr.grad, 'coilpos'),
        gradCoils    = physioset.sensors.coils(...
            'Cartesian',    hdr.grad.coilpos, ...
            'Orientation',  hdr.grad.coilori, ...
            'Weights',      hdr.grad.tra(gradIdx, :));
        gradSensors  = physioset.sensors.meg(...
            'Coils',        gradCoils, ...
    ...
    elseif isfield(hdr.grad, 'pnt'),
        gradCoils = physioset.sensors.coils('Weights', hdr.grad.tra(gradIdx,:));
        gradSensors  = physioset.sensors.meg(...
            'Coils',        gradCoils, ...
...
if any(isPhys),
    ...
    physSensors = physioset.sensors.physiology(...
        'Label',    physLabels, ...
        'PhysDim',  hdr.unit(physIdx));
end

physioset.sensors.ixed = physioset.sensors.mixed(gradSensors, magSensors, eegSensors, physSensors);
...
% Create a physioset.sensors.meg object
% =========================================================================
if opt.verbose,
...
physiosetObj = physioset(opt.filename, n_dims, ...
    ...
    'Continuous',       true, ...
    'Event',            events, ...
    'Sensors',          physioset.sensors.ixed, ...
