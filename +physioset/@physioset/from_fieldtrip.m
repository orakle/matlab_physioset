...
if isfield(fStruct, 'grad'),
    physioset.sensors.bj = physioset.sensors.meg.from_fieldtrip(fStruct.grad, fStruct.label); 
elseif isfield(fStruct, 'elec'),
    physioset.sensors.bj = physioset.sensors.eeg.from_fieldtrip(fStruct.elec, fStruct.label);
else
    warning('physioset.from_fieldtrip:MissingSensorInformation', ...
        ['Fieldtrip structure does not contain sensor information:' ...
        'Assuming vanilla EEG physioset.sensors.]);
    physioset.sensors.bj = physioset.sensors.eeg.empty();
end
